...
// mobile search
const mobileSearch = document.querySelector('.search-input-mobile')
...
mobileSearch.addEventListener("focus", () => {
  ...
  console.log('mobile serach')

  window.onclick = function (event) {
    if (
      !event.target.classList.contains("mobile-search-result") &&
      !event.target.classList.contains("search-input-mobile")
    ) {
      if ((mobileSearchResult.display = "block")) {
...
mobileSearch.addEventListener("input", mobileSearchFunction);
function mobileSearchFunction() {
  console.log('function')
  const celebrityMobile = document.getElementsByClassName("celebrity-search-id-mobile");
  const searchReturnMobile = document.getElementsByClassName("recommended-mobile");
  const filter = searchClick.value.toUpperCase();
  const searchTittleMobile = document.querySelector(".mobile-search-tittle");

  if (searchClick.value == "") {
    searchTittleMobile.style.display = "block";
  } else {
    searchTittleMobile.style.display = "none";
  }

  for (i = 0; i < celebrityMobile.length; i++) {
    const celebrityName =
